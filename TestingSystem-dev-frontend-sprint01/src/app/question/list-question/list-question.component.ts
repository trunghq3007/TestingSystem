...
export class ListQuestionComponent implements OnInit {
  ...
  ngOnInit() {
    this.dataSource.sortingDataAccessor = (item, property) => {
      switch (property) {
        case 'category':
          return item.questionCategory.categoryName;
        case 'create_by':
          return item.userQuestion.fullName;
        case 'date':
          return item.dateCreated;
        case 'level':
          return item.questionLevel.levelName;
        case 'content':
          return item.content;
        case 'tag':
          return item.questionTag.tagName;
        default:
    ...
    this.service.getType().subscribe(type => (this.listType = type));
    this.service.getAllLvl().subscribe(lvl => (this.listLvl = lvl));
    this.service
      .getAllCategory()
      .subscribe(category => (this.listCategory = category));
    this.service.getAllTag().subscribe(tag => (this.listTag = tag));
  }
  loadListQuestion() {
    if (this.isSearching) {
      this.service
      ...
      this.service.countSearchQuestion(this.searchStr).subscribe(count => {
        (this.tabAllQuestion.entities = +count.headers.get(
          'CountSearchQuestion'
        )),
          (this.maxPage = Math.trunc(
            +count.headers.get('CountSearchQuestion') /
              this.tabAllQuestion.sizeOfPage
          )),
      ...
      this.selection = new SelectionModel<Question>(true, []);
      this.service.getQuestionSum().subscribe(sum => {
        (this.tabAllQuestion.entities = +sum.headers.get('SumQuestion')),
          (this.maxPage = Math.ceil(
            +sum.headers.get('SumQuestion') / this.tabAllQuestion.sizeOfPage
          )),
  ...
  updateMuiltiQestion() {
    if (this.selection.selected.length == 0) {
      ...
      this.selection.selected.forEach(element => {
        ...
        this.service.updateMutilQuestion1(element).subscribe(
          success => {
