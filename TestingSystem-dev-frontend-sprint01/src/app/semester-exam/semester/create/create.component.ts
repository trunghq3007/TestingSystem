...
export class CreateComponent implements OnInit {
   ...
   constructor(private fb: FormBuilder,
     private router: Router,
      ...
      this.ckeConfig = { extraPlugins: 'divarea', height: 110,
      allowedContent: false, forcePasteAsPlainText: true, fontSize_defaultLabel: 22 };
   }
      ...
      try {
         ...
         if (semesterExam.startTime.getTime() < semesterExam.endTime.getTime()) {
            this.service.saveOne('semesterexam/add', semesterExam).subscribe(data => {
               this.router.navigateByUrl('cms/semester');
            });
         } else {
            let timerInterval
            Swal.fire({
