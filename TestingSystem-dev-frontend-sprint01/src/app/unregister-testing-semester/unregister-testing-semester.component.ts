...
@Component({
  selector: 'app-unregister-testing-semester',
  templateUrl: './unregister-testing-semester.component.html',
  styleUrls: ['./unregister-testing-semester.component.css']
})
export class UnregisterTestingSemesterComponent implements OnInit {
  private start_time: Date;
   ...
   onSubmit(result: object) {
      ...
      this.userTestingService.submitResultTest( this.test.testID, testingData).subscribe(
         ...
         () => {
            this.route.navigate(['unregister/semester', this.semesterId, 'result', this.testId], { replaceUrl: true });
            localStorage.setItem(this.startTimeLocalstorage, '');
         });
   }
